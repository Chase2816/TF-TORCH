...
def resize_predictions_detection_masks(image_shape=('height', 'width', 'channels'), mask_threshold=0.):
    ...
    # preds = requests.post("http://172.20.112.102:8521/v1/models/model-hot_group:predict", json=data)
    preds = requests.post("http://172.20.112.102:6001/v1/models/maskrcnn:predict", json=data)
...
img_dir = r"E:\data\bzz_data\bzz_lg_01_800_"
for filename in os.listdir(img_dir):
    # if filename.split(".") != "jpg":continue
    ...
    for mask in detection_masks:
        ...
        # pil_solid_color.show()
        mask_pil = Image.fromarray(np.uint8(255.0 * alpha * mask)).convert("L")
        # mask_pil.show()
        img_bgr = cv2.cvtColor(np.asarray(mask_pil), cv2.COLOR_RGB2BGR)
        gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)

        contours, h = cv2.findContours(gray, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        print(contours)
        ret = cv2.drawContours(img_bgr, contours, -1, (0, 0, 255), 2)
        ...
        img_bgr_ = cv2.cvtColor(np.asarray(img_pil), cv2.COLOR_RGB2BGR)

        # img_pil.show()
        # print(img_pil)
        cv2.drawContours(img_bgr_, [box], 0, (255, 0, 0), 5)

        # cv2.imshow("mask", img_bgr_)
    ...
    colors = ["red","yellow","blue","white","black","red","yellow","blue","white","black","red","yellow","blue","white","black","red","yellow","blue","white","black",
              "red","yellow","blue","white","black","red","yellow","blue","white","black","red","yellow","blue","white","black","red","yellow","blue","white","black",
              "red","yellow","blue","white","black","red","yellow","blue","white","black","red","yellow","blue","white","black","red","yellow","blue","white","black",
              "red","yellow","blue","white","black","red","yellow","blue","white","black","red","yellow","blue","white","black","red","yellow","blue","white","black",
              "red","yellow","blue","white","black","red","yellow","blue","white","black","red","yellow","blue","white","black","red","yellow","blue","white","black",
              "red","yellow","blue","white","black","red","yellow","blue","white","black","red","yellow","blue","white","black","red","yellow","blue","white","black"]
    ...
    for i, boxes in enumerate(detection_boxes):
        ...
        draw.rectangle((boxes[0], boxes[1], boxes[2], boxes[3]), outline=randomcolor(),width=3)
        draw.text((boxes[0], boxes[1]), str(i), font=setFont, fill=randomcolor())
    # img_pil.show()
    if not os.path.exists(r"E:\data\bzz_data\bzz_lg_01_800_/outputs"):
        os.mkdir(r"E:\data\bzz_data\bzz_lg_01_800_/outputs")
    img_pil.save(r"E:\data\bzz_data\bzz_lg_01_800_/outputs/"+os.path.basename(img_path))
