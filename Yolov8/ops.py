...
def non_max_suppression(
    ...
    nc = nc or (prediction.shape[1] - 4)  # number of classes 1
    nm = prediction.shape[1] - nc - 4  # 32
    mi = 4 + nc  # mask start index  5
    xc = prediction[:, 4:mi].amax(1) > conf_thres  # candidates [1,8400]

    # Settings
    # min_wh = 2  # (pixels) minimum box width and height
    time_limit = 0.5 + max_time_img * bs  # seconds to quit after 0.55  max_time_img=0.05
    redundant = True  # require redundant detections
    multi_label &= nc > 1  # multiple labels per box (adds 0.5ms/img) False
    merge = False  # use merge-NMS

    t = time.time()
    output = [torch.zeros((0, 6 + nm), device=prediction.device)] * bs # [tensor([], size=(0, 38))]
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        # x[((x[:, 2:4] < min_wh) | (x[:, 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x.transpose(0, -1)[xc[xi]]  # confidence  x=[37,8400] x.transpose=[8400,37]  x = [91,37]
        ...
        box, cls, mask = x.split((4, nc, nm), 1) # [91,4] [91,1] [91,32]
        box = xywh2xyxy(box)  # center_x, center_y, width, height) to (x1, y1, x2, y2)
        if multi_label:
            i, j = (cls > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, 4 + j, None], j[:, None].float(), mask[i]), 1)
        else:  # best class only
            conf, j = cls.max(1, keepdim=True)  # [91,1]
            x = torch.cat((box, conf, j.float(), mask), 1)[conf.view(-1) > conf_thres] # [91,38]
        ...
        # Check shape
        n = x.shape[0]  # number of boxes  91
        if not n:  # no boxes
            continue
        # max_nms=30000  [91,38]
        ...
        # Batched NMS
        c = x[:, 5:6] * (0 if agnostic else max_wh)  # classes max_wh=7680  [91,1]
        boxes, scores = x[:, :4] + c, x[:, 4]  # boxes (offset by class), scores
        i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS 9
...
def process_mask(protos, masks_in, bboxes, shape, upsample=False):
    ...
    c, mh, mw = protos.shape  # CHW
    ih, iw = shape 
    # [9, 32] @ [32, 160, 160]-->[32, 25600] == [9, 160, 160]
    masks = (masks_in @ protos.view(c, -1)).sigmoid().view(-1, mh, mw)  # CHW
